...
def draw_rate(gt_file, gt_file_1,  directory):
    gt_x = []
    gt_y = []
    idx = 0
    ticks = []
    ...
    idx = 0
    ticks = []
    with open(gt_file_1) as f:
        for rate in f:
            if float(rate) > 0.0:
                gt_x_1.append(idx)
                if float(rate) > 50:
                    gt_y_1.append(float(rate)/1000.0)
                else:
                    gt_y_1.append(float(rate))
                ticks.append(str(idx)+'~'+str(idx+3)+' ')
                idx+=3

    gt_x_1.pop(len(gt_x_1)-1)
    gt_y_1.pop(len(gt_y_1)-1)

    plt.figure()
    plt.title('Flow Rate')
    plt.ylabel('Rate (Mbits/sec)')
    plt.xlabel('Time')
    gt_x_1 = np.array(gt_x_1)
    gt_x = np.array(gt_x)
    plt.bar(gt_x_1, gt_y_1, 0.4, alpha=.4, label='Without Detectionn', color="red")
    plt.bar(gt_x+0.4, gt_y, 0.4, alpha=.4, label='With Detectionn', color="green")
...
if __name__ == "__main__":
    ...
    draw_rate('p_result.txt', '3_result.txt',  '.')
